...
export async function processDue(db: Store, send: ReminderSend, now: string): Promise<ProcessResult> {
  const due = await db.listDue(now);
  let sent = 0;
  let failed = 0;
  for (const r of due) {
    try {
      await send(r);
      await db.markSent(r.id, now);
      sent++;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      await db.markFailed(r.id, message, now);
      failed++;
    }
  }
  return { sent, failed };
...
/** Starts a repeating loop that processes due reminders. Local dev only. */
